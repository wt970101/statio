...
def get_data(keyword):
    service = Service(r"C:\MyProject\webapp\driver\chromedriver.exe")
        ...
        for item in items_9x9:
            ...
            for r in items_rakuten:
                if r['name'] == item['name'] and r['price'] is not None:
                    prices.append({'site': 'rakuten', 'price': r['price'], 'url': r['url']})
            # 9*9 價格
            prices.append({'site': '9x9', 'price': item['price'], 'url': item['url']})
